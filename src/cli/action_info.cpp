...
int ActionInfo::exec(const OptionList& opts) {
	...
	if (resources) {
		...
		for (auto i = 0; i < rsrcCount; ++i) {
			...
			fmt::print(FMT_STRING("    0x{:X} ({:c}{:c}{:c}) - {} bytes ({:1f} KiB)\n"),
				type, type & 0xFF, (type>>8) & 0xFF, (type>>16) & 0xFF, sz, sz/1024.f);
		}
	}
	
	fmt::print(FMT_STRING("{:2f} KiB image data ({:2f} MiB)\n"),
		file_->GetSize() / 1024.f, file_->GetSize() / (1024.f*1024.f));
...
void ActionInfo::compact_info() {
	fmt::print(FMT_STRING("VTF {}.{}, {} x {} x {}, {} frames, {} mipmaps, {} faces, image format {}\n"),
		file_->GetMajorVersion(), file_->GetMinorVersion(),
