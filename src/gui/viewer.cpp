...
bool ViewerMainWindow::load_file( VTFLib::CVTFFile *file, const char *path )
{
	setWindowTitle(
		fmt::format( FMT_STRING( "VTFView - [{}]" ), str::get_filename( path ) ).c_str() );
...
void ViewerMainWindow::setup_menubar()
{
	...
	viewMenu->addAction(
		QIcon::fromTheme( "zoom-in", QIcon( ":/zoom-plus.svg" ) ), "Zoom In", [this]()
		{
			viewer_->zoom( .1f );
		},
		QKeySequence::ZoomIn );
	viewMenu->addAction(
		QIcon::fromTheme( "zoom-out", QIcon( ":/zoom-minus.svg" ) ), "Zoom Out", [this]()
		{
			viewer_->zoom( -.1f );
		},
...
void ImageSettingsWidget::setup_ui( ImageViewWidget *viewer )
{
	...
	connect( startFrame_, &QSpinBox::textChanged, [this]( const QString & )
			 {
				 if ( !file_ )
					 return;
